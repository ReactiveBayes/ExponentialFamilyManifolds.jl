...
struct WithMetric{𝔽, T, Mode<:MetricMode, M<:NaturalParametersManifold} <: AbstractDecoratorManifold{𝔽}
    man::M
end
ManifoldsBase.decorated_manifold(W::WithMetric) = W.man

# convenience constructors
with_natural_metric(M::NaturalParametersManifold{F}) where {F} = begin
...
@inline ManifoldsBase.get_forwarding_type(::WithMetric{F, T, NaturalMetric}, ::Any, ::typeof(inner)) where {F, T} = _STOP
@inline ManifoldsBase.get_forwarding_type(::WithMetric{F, T, NaturalMetric}, ::Any, ::typeof(norm)) where {F, T} = _STOP
@inline ManifoldsBase.get_forwarding_type(::WithMetric{F, T, NaturalMetric}, ::typeof(inner)) where {F, T} = _STOP
@inline ManifoldsBase.get_forwarding_type(::WithMetric{F, T, NaturalMetric}, ::typeof(norm))  where {F, T} = _STOP
...
function Base.convert(
    ::Type{ExponentialFamilyDistribution}, M::WithMetric, p
)   
    natural_manifold = M.man
    return ExponentialFamilyDistribution(
        exponential_family_typetag(natural_manifold), transform_back(natural_manifold, p), getconditioner(natural_manifold), nothing
    )
end
